...
from codeguard.scanner.workspace import (
    JvmClasspathArtifact,
    WorkspaceManifest,
    WorkspaceRepository,
)
from codeguard.semantic.jvm import JvmBuildDiscoverer
from codeguard.semantic.jvm_bytecode import JvmClasspathSnapshot
...
class JvmClasspathBuildTarget(BaseModel):
    ...
    build_root_key: str
    expected_output: str = "codeguard-classpath.zip"
...
class JvmClasspathPlanner:
    ...
    def _plan_repository(
        ...
        for index, (root, systems) in enumerate(sorted(by_root.items()), start=1):
            ...
            output = "codeguard-classpath.zip"
            targets.append(
        ...
        warnings.append(
            "network is disabled; the pinned image must contain Python/CodeGuard, "
...
class JvmClasspathBundleMaterializer:
    ...
    def materialize(
        ...
        *,
        ...
        temporary = Path(tempfile.mkdtemp(prefix=".codeguard-classpath-", dir=target.parent))
        try:
