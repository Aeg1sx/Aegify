"""CodeGuard SAST CLI powered by Typer."""
...
from codeguard.models import ScanProgress, ScanResult, Severity

app = typer.Typer(
    name="codeguard",
    help="CodeGuard SAST - Next-gen static application security testing",
...
def scan(
    ...
    upload_dashboard: Annotated[
        bool,
        typer.Option("--upload-dashboard", help="Upload SARIF results to CodeGuard dashboard"),
    ] = False,
    ...
    # Load config
    if config_path:
        cfg = CodeGuardConfig.from_yaml(config_path)
    else:
        cfg = CodeGuardConfig.load(target if target.is_dir() else target.parent)
    ...
    console.print(
        Panel(
            f"[bold]CodeGuard SAST v{__version__}[/bold]\n"
            f"Target: {target}\n"
            f"Severity: >= {severity}\n"
    ...
    if llm and cfg.anthropic_api_key and result.findings:
        ...
        from codeguard.llm.verifier import LLMVerifier
    ...
    if upload_defectdojo:
        ...
        if not dojo_url or not dojo_token:
            console.print(
                ...
                "CODEGUARD_REPORTING__DEFECTDOJO_URL and "
                "CODEGUARD_REPORTING__DEFECTDOJO_TOKEN env vars "
                "(or .codeguard.yml config)[/red]"
            )
        else:
...
def version() -> None:
    ...
    console.print(f"CodeGuard SAST v{__version__}")
...
def scan_workspace(
    manifest: Annotated[
        Path,
        typer.Argument(
            help="Path to a versioned CodeGuard workspace YAML manifest",
            exists=True,
            dir_okay=False,
        ),
    ],
    ...
    cfg = CodeGuardConfig.load(manifest.parent)
    cfg.rules.severity_threshold = severity
...
def index_scip_java(
    ...
    from codeguard.harness import DockerVerificationExecutor
    from codeguard.harness.docker import HarnessUnavailableError
    from codeguard.semantic import ScipJavaPlanner
...
def export_jvm_classpath(
    ...
    from codeguard.harness import DockerVerificationExecutor
    from codeguard.harness.docker import HarnessUnavailableError
    from codeguard.semantic import (
...
def audit_rule_files(
    ...
    if json_output:
        ...
        table = Table(title="CodeGuard Rule Audit")
...
def verify_plan(
    ...
    from codeguard.harness import DockerVerificationExecutor, VerificationPlan
    from codeguard.harness.docker import HarnessUnavailableError
...
def verify_http(
    ...
    from codeguard.harness import HttpVerificationExecutor, HttpVerificationPlan
    from codeguard.harness.docker import HarnessUnavailableError
...
def verify_browser(
    ...
    from codeguard.harness import BrowserVerificationExecutor, BrowserVerificationPlan
    from codeguard.harness.docker import HarnessUnavailableError
...
def verify_proxy(
    ...
    from codeguard.harness import ProxyVerificationExecutor, ProxyVerificationPlan
    from codeguard.harness.docker import HarnessUnavailableError
...
def scan_pr(
    ...
    # Load config
    cfg = CodeGuardConfig.load(target)
    cfg.rules.severity_threshold = severity
    cfg.llm.enabled = llm
    cfg.llm.model = model

    if llm and not cfg.anthropic_api_key:
        console.print("[red]CODEGUARD_ANTHROPIC_API_KEY env var is required for scan-pr[/red]")
        raise typer.Exit(code=2)

    # Step 1: Resolve changed files
    from codeguard.scanner.diff_resolver import DiffResolver
    ...
    console.print(
        Panel(
            f"[bold]CodeGuard SAST v{__version__} — PR Scan[/bold]\n"
    ...
    if llm and result.findings:
        ...
        from codeguard.llm.pr_verifier import PRVerifier
    ...
    # Step 5: Output SARIF
    sarif_path = output_file or Path("codeguard-results.sarif")
    _output_sarif(result, sarif_path, call_graph=engine._last_call_graph)

    # Step 6: Output PR comment markdown
    from codeguard.reporter.github import generate_pr_comment
    ...
    md_path = comment_file or Path("codeguard-pr-comment.md")
