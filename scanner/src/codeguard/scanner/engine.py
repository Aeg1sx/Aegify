...
from codeguard.rules.registry import load_builtin_rules, load_custom_rules
...
from codeguard.scanner.context import ContextAnalyzer
from codeguard.scanner.dataflow import DataflowAnalyzer
from codeguard.scanner.endpoint_detector import EndpointDetector
from codeguard.scanner.openapi_parser import find_openapi_files, parse_openapi_file
from codeguard.storage import InMemoryBackend, StorageBackend
from codeguard.storage.hasher import compute_file_hash
...
class ScanEngine:
    ...
    def scan_workspace(self, manifest_path: Path) -> ScanResult:
        ...
        try:
            ...
            from codeguard.adapters import ExternalAnalysisBundle, ExternalAnalysisImporter
            ...
            from codeguard.runtime import RuntimeEvidenceBundle, RuntimeEvidenceImporter
    ...
    def _run_pipeline(
        ...
        from codeguard.scanner.call_graph import CallGraphNode as CGNode
    ...
    def _overlay_program_graph(
        ...
        for source, target, attributes in call_edges:
            ...
            program_graph.add_edge(
                ...
                provider=attributes.get("provider", "codeguard-call-graph"),
        ...
        for source, target, attributes in sorted(
            ...
            evidence = {
                ...
                "column": column,
                "file_path": file_path,
                "provider": attributes.get("provider", "codeguard-source-icfg"),
    ...
    def _link_runtime_observations(self, result: ScanResult) -> int:
        ...
        from codeguard.models import AttackSurfaceLink, FrontendCall
    ...
    def _attach_finding_provenance(
        ...
        for finding in findings:
            ...
            if finding.rule_id not in rule_digests:
                if rule is None:
                    producer = "codeguard.unknown-rule"
                    rule_payload = finding.rule_id
                else:
                    producer = f"codeguard.{type(rule).__name__}"
    ...
    def _attach_attack_surface_provenance(self, result: ScanResult) -> None:
        ...
        for link in result.attack_surface_links:
            ...
            link.provenance = EvidenceProvenance(
                producer=(
                    "codeguard.RuntimeEvidenceCorrelator"
                    if link.source_kind == "runtime_observation"
                    else "codeguard.AttackSurfaceAnalyzer"
                ),
                producer_version=__version__,
                analysis_kind=(
                    "dynamic-correlation"
    ...
    def _create_storage(self) -> StorageBackend:
        ...
        if backend == "sqlite":
            ...
            from codeguard.storage.postgresql import PostgreSQLBackend

            return PostgreSQLBackend(self.config.storage.db_url)
        elif backend == "s3":
            from codeguard.storage.s3 import S3Backend
