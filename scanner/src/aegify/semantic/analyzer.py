...
from aegify.graph_types import SemanticGraph
from aegify.models import FileAST, SemanticAnalysisSummary, SemanticRelationship
from aegify.scanner.workspace import WorkspaceManifest
from aegify.semantic.jvm import (
    JvmBuildDiscoverer,
    JvmModuleGraphAnalyzer,
    JvmSemanticAnalyzer,
)
from aegify.semantic.jvm_bytecode import JvmBytecodeImporter
from aegify.semantic.jvm_dependencies import JvmDependencyAnalyzer
from aegify.semantic.scip import ScipImport, ScipImporter, ScipImportError
from aegify.semantic.scip_symbol import ScipPackageCoordinate, parse_scip_symbol
    ...
    def analyze(
        ...
        for repository in manifest.repositories:
            ...
            if module_graph.modules:
                providers.add("aegify-jvm-build")
            for module in module_graph.modules:
                graph.add_node(module, kind="jvm-module", provider="aegify-jvm-build")
        ...
        if packages:
            providers.add("aegify-scip-package-resolver")
        ...
        for package in packages:
            graph.add_node(
                ...
                provider="aegify-scip-package-resolver",
            )
        ...
        if dependencies.artifacts:
            providers.add("aegify-jvm-dependency")
        ...
        for repository in manifest.repositories:
            for classpath_artifact in repository.jvm_classpath_snapshots:
                try:
                    ...
                    continue
                providers.add("aegify-jvm-bytecode")
        ...
        if jvm.types:
            providers.add("aegify-jvm-source")
        if (
            ...
            providers.add("aegify-jvm-points-to")
        for type_id in jvm.types:
            graph.add_node(type_id, kind="jvm-type", provider="aegify-jvm-source")
