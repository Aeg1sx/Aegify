...
class JvmClasspathExporter:
    ...
    def _maven_records(self, root: Path, token: str) -> list[ClasspathRecord]:
        relative_output = f".aegify-classpath/{token}.txt"
        ...
        for output in sorted(root.rglob(f".aegify-classpath/{token}.txt")):
    ...
    def _gradle_records(self, root: Path, token: str) -> list[ClasspathRecord]:
        task_name = f"aegifyExportClasspath{token}"
        record_root = root / ".aegify-classpath" / token
        user_home = self._runtime_cache("gradle")
        script = self._gradle_init_script(task_name, record_root)
        with tempfile.TemporaryDirectory(prefix="aegify-classpath-") as directory:
    ...
    def _runtime_cache(name: str) -> Path:
        destination = Path(tempfile.gettempdir()) / f"aegify-{name}"
        source = Path("/opt/aegify-cache") / name
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output", default="aegify-classpath.zip")
    return parser
