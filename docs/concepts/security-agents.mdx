...
Offline tests use scripted API transports and owned inert CLI programs. A separate
[live Codex acceptance run](/project/beta-quality-acceptance) covers six roles and
twelve calls on an owned inert fixture. It validates protocol and citations, not
review accuracy or isolation. Anthropic retains reported usage; other provider
accounting and all monetary cost remain unverified.
...
The CLI publishes its final artifact atomically and returns exit code `3` for
partial review or pending approval. In the development checkout, `--checkpoint`
records each provider dispatch before calling and each validated reply before
returning. `--resume` replays completed replies while rebuilding deterministic
facts and read-only tool evidence. See the [recovery guide](/project/beta-quality-acceptance#controlled-agent-recovery).
The published `v0.3.0-beta.1` does not include this checkpoint feature.
