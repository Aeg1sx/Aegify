...
description: "Inject Aegify secrets safely with local environment files, GitHub OIDC, or HashiCorp Vault."
icon: "key-round"
---
...
[Encrypted backups](/operations/backup-recovery) use a separate `AEGIFY_BACKUP_KEY`,
injected only into the operator command. Retain the original `ENCRYPTION_SECRET`
and the key used for each retained archive in a protected secret store separate
from archive storage. Rotation must preserve the ability to decrypt retained data.
